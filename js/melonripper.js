...
(function init() {
    ...
    if(target != null) {
        ...
        var pageTotal = parseInt(document.body.getElementsByClassName('var-page-total')[0].textContent);
        imageArray = Array.apply(null, Array(pageTotal)).map(function () {});

        // delete view-protection, just for better visual
        document.body.getElementsByClassName('view-protection')[0].remove();
...
function savePages(pages) {
    var pageIndex = parseInt(document.body.getElementsByClassName('var-page-current')[0].textContent)-1;
...
function saveToZip() {
    ...
    for(var i = 0; i < imageArray.length; i++) {
        if(imageArray[i]) {
            ...
            zip.file((i+1) + '.png', imageArray[i], {base64: true});
        } else {
